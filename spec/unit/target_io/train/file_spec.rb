...
RSpec.describe TargetIO::TrainCompat::File do
  ...
  describe ".readlink" do
    before do
      ...
      allow(transport_connection).to receive(:file).with("/etc/alternatives/ruby").and_return(train_file)
      allow(transport_connection).to receive(:run_command)
        .with("readlink /etc/alternatives/ruby")
        .and_return(double(stdout: "/usr/bin/ruby3.0\n"))
    end

    it "returns the symlink target" do
      expect(described_class.readlink("/etc/alternatives/ruby")).to eq("/usr/bin/ruby3.0")
    end
    ...
    context "when path is not a symlink" do
      ...
      it "raises Errno::EINVAL" do
        expect { described_class.readlink("/etc/alternatives/ruby") }.to raise_error(Errno::EINVAL)
      end
    end
  end
  ...
  describe ".method_missing - non-IO passthrough (to ::File)" do
    it "delegates :basename to ::File" do
      expect(described_class.basename("/foo/bar.rb")).to eq("bar.rb")
    end
    ...
    it "delegates :dirname to ::File" do
      expect(described_class.dirname("/foo/bar.rb")).to eq("/foo")
    end

    it "delegates :extname to ::File" do
      expect(described_class.extname("/foo/bar.rb")).to eq(".rb")
    end
  end
