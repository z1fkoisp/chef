...
RSpec.describe TargetIO::TrainCompat::FileUtils do
  ...
  describe ".chmod" do
    ...
    it "skips execution when noop: true" do
  ...
  describe ".chown" do
    it "runs chown with user and group" do
      expect(transport_connection).to receive(:run_command).with("chown alice:staff /etc/app")
      described_class.chown("alice", "staff", "/etc/app")
    end

    it "omits group when nil" do
      expect(transport_connection).to receive(:run_command).with("chown alice /etc/app")
      described_class.chown("alice", nil, "/etc/app")
    end

    it "omits user when nil" do
      expect(transport_connection).to receive(:run_command).with("chown :staff /etc/app")
      described_class.chown(nil, "staff", "/etc/app")
    end

    it "skips execution when noop: true" do
      expect(transport_connection).not_to receive(:run_command)
      described_class.chown("alice", "staff", "/etc/app", noop: true)
    end
  end
  ...
  describe ".rm" do
    it "removes a file" do
      expect(transport_connection).to receive(:run_command).with("rm /tmp/file")
      described_class.rm("/tmp/file")
    end
  ...
  describe ".touch" do
    it "touches a file" do
      expect(transport_connection).to receive(:run_command).with("touch /tmp/file")
      described_class.touch(["/tmp/file"])
    end

    it "accepts nocreate option" do
      expect(transport_connection).to receive(:run_command).with("touch -c /tmp/file")
      described_class.touch(["/tmp/file"], nocreate: true)
    end

    it "skips execution when noop: true" do
      expect(transport_connection).not_to receive(:run_command)
      described_class.touch(["/tmp/file"], noop: true)
    end
  end
